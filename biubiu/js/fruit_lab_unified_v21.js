...
function labStatPreviewV21(t, lv) {
  const role = t.role || 'tank';
  const g = (typeof ROLE_GROWTH !== 'undefined' ? ROLE_GROWTH : null)?.[role] || { atk: [0,1], hp: [0,1] };
  const scale = (typeof FRUIT_SCALE !== 'undefined' ? FRUIT_SCALE : null)?.[t.id] || { atk: 1.0, hp: 1.0 };
  const atk = Math.round(t.atk * (g.atk[lv] || 1) * scale.atk);
  const hp = Math.round(t.hp * (g.hp[lv] || 1) * scale.hp);
  return { atk, hp };
}
...
function labDoUpgradeV21(id, stat) {
  if (stat === 'startlv') {
    const lv = getStartingLv(meta, id);
    if (lv >= STARTING_LV_MAX) return;
    const need = [0, 3, 10, 25, 50][lv];
    const have = (meta.fragments && meta.fragments[id]) || 0;
    if (have < need) return;
    meta.fragments[id] = have - need;
    meta.startingLvs = meta.startingLvs || {};
    meta.startingLvs[id] = lv + 1;
    saveMeta();
    refreshGold();
    renderFruitLabV21();
    setTimeout(() => {
      const card = document.querySelector(`[data-fruit-id="${id}"] .lab-startlv-v21`);
      if (card) { const f = document.createElement('span'); f.textContent = '+1'; f.className = 'lab-flash-v21'; card.appendChild(f); setTimeout(() => f.remove(), 400); }
    }, 50);
    return;
  }
...
function labToggleDeckV21(id) {
  ...
  if (meta.deck.includes(id)) {
    ...
    if (meta.deck.length >= DECK_SIZE) meta.deck.pop();
    meta.deck.push(id);
  }
  saveMeta();
  refreshGold();
...
function renderFruitLabCardV21(id, unlocked) {
  ...
  const startLv = getStartingLv(meta, id);
  const startNeed = [0, 3, 10, 25, 50][startLv];
  const startHave = (meta.fragments && meta.fragments[id]) || 0;
  const s = labStatPreviewV21(t, startLv);
  const sMax = labStatPreviewV21(t, MAX_LEVEL);
  const el = document.createElement('div');
  el.className = 'fruit-lab-card-v21' + (on ? ' on' : '') + (locked ? ' locked' : '');
  el.dataset.fruitId = id;
  el.innerHTML = `
    <div class="lab-card-top-v21">
      ...
      <div class="lab-status-v21">${locked ? `第${unlockLevelFor(id)}关` : on ? '上阵中' : full ? '替换' : '上阵'}</div>
    </div>
    <div class="lab-info-grid-v21"><span>攻 ${s.atk}→${sMax.atk}</span><span>防 ${s.hp}→${sMax.hp}</span><span>出兵 ${(SPAWN_COOLDOWNS[startLv]).toFixed(1)}→${(SPAWN_COOLDOWNS[MAX_LEVEL]).toFixed(1)}s</span></div>
    ...
    <div class="lab-actions-v21">
      <button class="lab-btn-v21 ${on ? 'off' : locked || full ? 'lock' : ''}" data-action="deck">${locked ? '未解锁' : on ? '下阵' : full ? '替换' : '上阵'}</button>
      <div class="lab-startlv-v21">
        <b>起始等级 Lv.${startLv}</b>
        <small>碎片 ${startHave}/${startNeed} · 召唤即 Lv.${startLv}</small>
        <button data-action="startlv" ${startLv >= STARTING_LV_MAX || startHave < startNeed ? 'disabled' : ''}>${startLv >= STARTING_LV_MAX ? 'MAX' : '升级'}</button>
      </div>
    </div>`;
  el.querySelector('[data-action="deck"]').addEventListener('click', () => labToggleDeckV21(id));
  const startBtn = el.querySelector('[data-action="startlv"]');
  if (startBtn) startBtn.addEventListener('click', () => labDoUpgradeV21(id, 'startlv'));
  return el;
}
