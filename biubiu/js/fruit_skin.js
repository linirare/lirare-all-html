...
function drawHUD() {
  ...
  /* 左上：SP/召唤消耗 */
  drawPanel(8, LAYOUT.enemyInfoY, 148, 22, 10, 'rgba(255,255,255,0.58)', 'rgba(83,201,106,0.18)');
  ctx.font = 'bold 11px sans-serif';
  ctx.textAlign = 'left';
  ctx.fillStyle = state.sp > 0 ? '#c78200' : '#8aad6a';
  ctx.fillText(String.fromCodePoint(0x1F4A7) + state.summonCostCounter + ' ' + String.fromCodePoint(0x26A1) + state.sp, 16, LAYOUT.enemyInfoY + 16);
  ctx.font = '9px sans-serif';
  ctx.fillStyle = THEME.textDim;
  ctx.fillText(String.fromCodePoint(0x654C) + String.fromCodePoint(0x26A1) + state.enemySp, 108, LAYOUT.enemyInfoY + 16);

  /* 右上：时间 */
  const timeX = PAUSE_RECT.x - 68;
  drawPanel(timeX - 2, LAYOUT.enemyInfoY, 66, 22, 10, 'rgba(255,255,255,0.56)', 'rgba(77,182,255,0.18)');
  ctx.font = 'bold 12px sans-serif';
  ctx.textAlign = 'center';
  ctx.fillStyle = '#4f6a31';
  ctx.fillText(String.fromCodePoint(0x23F1) + elapsed + 's', timeX + 31, LAYOUT.enemyInfoY + 16);

  /* 我方城墙HP */
  const wallW = W * 0.76, wallX = (W - wallW) / 2;
  ctx.font = 'bold 10px sans-serif';
  ctx.textAlign = 'left';
  ctx.fillStyle = THEME.safe;
  drawPanel(wallX + 4, LAYOUT.playerWallY - 16, 62, 14, 6, 'rgba(255,255,255,0.50)', 'rgba(83,201,106,0.20)');
  ctx.fillText(String.fromCodePoint(0x1F3F0) + state.playerWallHp + '/' + state.playerWallMax, wallX + 8, LAYOUT.playerWallY - 5);

  /* 敌方城墙HP */
  ctx.textAlign = 'right';
  ctx.fillStyle = THEME.accent;
  drawPanel(wallX + wallW - 64, LAYOUT.enemyWallY - 16, 62, 14, 6, 'rgba(255,255,255,0.50)', 'rgba(255,93,108,0.20)');
  ctx.fillText(String.fromCodePoint(0x1F3F0) + state.enemyWallHp + '/' + state.enemyWallMax, wallX + wallW - 6, LAYOUT.enemyWallY - 5);

  /* 战场中线：每路压力文本 */
  if (state.laneStats) {
    ctx.font = '9px sans-serif';
    ctx.textAlign = 'center';
    for (let i = 0; i < state.laneStats.length; i++) {
      const ls = state.laneStats[i];
      if (!ls || !ls.pressureText) continue;
      const lx = BOARD_X + i * (CELL + GAP) + CELL / 2;
      ctx.fillStyle = ls.danger > 0.6 ? 'rgba(255,93,108,0.72)' : ls.danger > 0.3 ? 'rgba(255,201,60,0.72)' : 'rgba(83,201,106,0.60)';
      ctx.fillText(ls.pressureText, lx, LAYOUT.fieldY + LAYOUT.fieldH / 2 - 6);
    }
  }
  ...
  roundRect(bx, by, barW, barH, 5);
  ctx.fill();
  if (total > 0) {
    ctx.fillStyle = THEME.safe;
    roundRect(bx, by, barW * (pCount / total), barH, 5);
    ctx.fill();
    ctx.fillStyle = THEME.accent;
    roundRect(bx + barW * (pCount / total), by, barW * (eCount / total), barH, 5);
    ctx.fill();
  }
  ctx.font = 'bold 10px sans-serif';
  ctx.textAlign = 'left';
  ctx.fillStyle = THEME.safe;
  ctx.fillText(String.fromCodePoint(0x1F349) + ' ' + pCount, bx, by - 3);
  ctx.textAlign = 'right';
  ctx.fillStyle = THEME.accent;
  ctx.fillText(eCount + ' ' + String.fromCodePoint(0x1FA60), bx + barW, by - 3);
}
